...
document.addEventListener('DOMContentLoaded', () => {
  const tabName = '@geekbar';
  let tabIndex = 0;
  let tabDeleting = false;

  function animateTabTitle() {
    if (!tabDeleting && tabIndex < tabName.length) tabIndex++;
    else if (tabDeleting && tabIndex > 0) tabIndex--;
    else if (tabIndex === tabName.length) {
      tabDeleting = true;
      setTimeout(animateTabTitle, 1400);
      return;
    } else if (tabIndex === 0) tabDeleting = false;
    document.title = tabName.slice(0, tabIndex) || ' '; 
    setTimeout(animateTabTitle, tabDeleting ? 100 : 180);
  }
  ...
  profileName.textContent = '@geekbar';
  profileBio.textContent = 'c++ dev and student';
